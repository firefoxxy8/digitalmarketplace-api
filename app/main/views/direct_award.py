...
from .. import main
from ... import db
from ...models import User, AuditEvent, ArchivedService, ProcessOutcome
...
def create_award_process_outcome(project_external_id, service_id):
    updater_json = validate_and_return_updater_request()
    ...
    if project.process_outcome is not None:
        abort(410, "Project {} already has a completed process outcome: {}".format(
            project_external_id,
            project.process_outcome.external_id,
        ))
    ...
    audit_event = AuditEvent(
        audit_type=AuditTypes.create_process_outcome,
        user=updater_json['updated_by'],
        data={
            "projectExternalId": project.external_id,
            "searchId": project.active_search.id,
            "archivedServiceId": archived_service.id,
            "result": process_outcome.result,
        },
